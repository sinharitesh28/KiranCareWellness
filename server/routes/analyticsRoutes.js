...
router.get('/sales', requireAuth, (req, res) => {
    ...
    const p1 = new Promise((resolve, reject) => {
        db.query(graphSql, params, (err, results) => err ? reject(err) : resolve(results));
    });
    const p2 = new Promise((resolve, reject) => {
        db.query(tableSql, params, (err, results) => err ? reject(err) : resolve(results));
    });

    Promise.all([p1, p2])
        .then(([graphData, tableData]) => {
...
router.get('/stock-status', requireAuth, (req, res) => {
    ...
    const p1 = new Promise((resolve, reject) => {
        db.query(statusSql, (err, results) => err ? reject(err) : resolve(results[0]));
    });
    const p2 = new Promise((resolve, reject) => {
        db.query(distributorSql, (err, results) => err ? reject(err) : resolve(results));
    });

    Promise.all([p1, p2])
        .then(([statusData, distributorData]) => {
            res.json({ success: true, statusData, distributorData });
        })
        .catch(err => {
...
// 3. ABC Analysis
router.get('/abc-analysis', requireAuth, (req, res) => {
    const { period, startDate, endDate } = req.query;
    const { dateCondition, params } = getDateCondition(period, startDate, endDate);
    ...
    db.query(abcSql, params, (err, results) => {
        if (err) {
